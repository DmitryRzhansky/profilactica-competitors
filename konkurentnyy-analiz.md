...
| Страниц в финальной архитектуре | 180 |
| из них P0 | 78 |
| из них P1 | 90 |
| из них P2 | 12 |
...
Полная таблица содержит **180 страниц**: 78 P0, 90 P1 и 12 условных P2. Ниже — базовый каркас, затем полный каталог всех страниц.
...
Общие методы, препараты и страницы «раскодирование от …» остаются самостоятельными страницами Москвы, но **не получают собственных ГЕО-матриц**. ГЕО создаётся только на уровне общего «кодирование от алкоголизма».
...
**Кодирование.** Хаб → форматы, методы, препараты, раскодирование. Метод/препарат → хаб + 3–5 альтернатив + парное `раскодирование от {тот же препарат}` + лечение алкоголизма + реабилитация + срыв после кодирования + цены + врач. Хаб раскодирования → все `раскодирование от …`. Страница `раскодирование от X` → хаб раскодирования + `кодирование: X` + 2–3 соседних раскодирования + срыв после кодирования + цены + врач. Методные/препаратные/раскодировочные страницы не получают ГЕО-сетку. ГЕО кодирования не ссылается на десятки препаратов.
...
Ниже вшиты все исходные списки отчёта: 180 страниц архитектуры уже приведены в разделе SILO; здесь — полный каталог 1260 запланированных ГЕО URL.
